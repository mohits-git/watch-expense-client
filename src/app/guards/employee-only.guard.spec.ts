...
import { employeeOnly } from './employee-only.guard';

describe('employeeOnly (guard)', () => {
  ...
  it('returns true when user is an employee', () => {
    authServiceMock.user.and.returnValue({ role: UserRole.Employee } as any);

    const result = TestBed.runInInjectionContext(() =>
      employeeOnly(route, segments),
    );

    expect(result).toBe(true);
  });

  it('returns false when user is not an employee', () => {
